...
1. 建立可追踪的项目事实与实验记录。
2. 先在 PC 侧跑通模型 A 的基础检测链路：`yolov8n.pt` COCO-pretrained -> smoke training -> predict -> ONNX/TFLite/INT8 候选导出。
3. 在 PC 侧确认 YOLO 检测输出、NMS、阈值和 `416x416` 到 VGA `640x480` 的坐标回映射。
4. 再进入 RA8P1/RUHMI/板端集成、自采桌面数据 fine-tune 和模型 B ROI 抓取矩形部分。
